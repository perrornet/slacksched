...
func BuildAgentContextAPISectionMarkdown(baseURL string) string {
	...
	b.WriteString("shell 中由进程注入：`SCHDULER_CONTEXT_API_TOKEN`。本会话基础地址：\n\n")
	b.WriteString("```\n")
	...
	b.WriteString("### Slack Web API 代理\n\n")
	b.WriteString("`POST " + baseURL + "/v1/slack/web-api/<方法名>`\n\n")
	b.WriteString("正文：`application/json` 或 `application/x-www-form-urlencoded`。响应与 Slack 官方 Web API 一致（含 `blocks`、`attachments` 等）。\n\n")
